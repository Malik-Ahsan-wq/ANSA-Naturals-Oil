...
export default function ProductBottle({ className }: { className?: string }) {
  return (
    <div className={`relative ${className ?? ""}`}>
      ...
      {/* Floating leaves */}
      <Leaf variant="a" className="absolute -top-4 -left-2 w-16 h-16 sm:w-20 sm:h-20 text-emerald-700/60 rotate-[-30deg] animate-float" />
      <Leaf variant="b" className="absolute top-10 -right-1 w-14 h-14 sm:w-18 sm:h-18 text-green-600/60 rotate-[40deg] animate-float-delay" />
