...
export default function CartSidebar() {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 right-0 z-[201] h-full w-full sm:max-w-[400px] bg-[#fafafa] shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${isCartOpen ? "translate-x-0" : "translate-x-full"}`}>
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-200 bg-white">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-[#111111] text-lg">Your Cart</h2>
            {getCartCount() > 0 && (
              <span className="bg-zinc-800 text-white text-xs font-bold px-2 py-0.5 rounded-full">{getCartCount()}</span>
            )}
          </div>
          <button onClick={closeCart} className="w-8 h-8 rounded-full bg-zinc-50 hover:bg-zinc-100 flex items-center justify-center text-zinc-700 transition-colors">
            <FaTimes size={13} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div key={item.product.id} className="flex gap-3 bg-white rounded-2xl p-3 border border-zinc-100 shadow-sm">
                <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 bg-zinc-50 flex items-center justify-center">
                  <Image src={item.product.image} alt={item.product.name} width={64} height={64} className="w-full h-full object-contain" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-sm text-[#111111] line-clamp-1">{item.product.name}</p>
                  <p className="text-xs text-zinc-400 mt-0.5">{brand.currency} {item.product.price.toLocaleString()} each</p>
                  <div className="flex items-center gap-2 mt-2">
                    <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)}
                      className="w-6 h-6 rounded-lg bg-zinc-50 border border-zinc-200 flex items-center justify-center text-zinc-700 hover:bg-zinc-100 transition-colors">
                      <FaMinus size={7} />
                    </button>
                    <span className="text-sm font-bold text-[#111111] w-4 text-center">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)}
                      className="w-6 h-6 rounded-lg bg-zinc-50 border border-zinc-200 flex items-center justify-center text-zinc-700 hover:bg-zinc-100 transition-colors">
                      <FaPlus size={7} />
                    </button>
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="border-t border-zinc-200 px-5 py-4 space-y-3 bg-white">
            <div className="flex justify-between items-center">
              <span className="text-zinc-500 font-medium text-sm">Subtotal</span>
              <span className="text-xl font-bold text-[#111111]">{brand.currency} {getCartTotal().toLocaleString()}</span>
            </div>
            <p className="text-xs text-zinc-500 flex items-center gap-1.5"><FaLeaf/> Free nationwide delivery applied at checkout</p>
            <button onClick={placeOrder}
              className="w-full bg-gradient-to-r from-[#111111] to-[#333333] hover:shadow-lg text-white py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md">
              <FaWhatsapp className="text-lg" /> Place Order via WhatsApp
            </button>
            <Link href="/cart" onClick={closeCart}
              className="w-full bg-zinc-50 hover:bg-zinc-100 text-zinc-700 py-3 rounded-xl font-bold text-sm flex items-center justify-center transition-all">
              View Full Cart
            </Link>
          </div>
        )}
